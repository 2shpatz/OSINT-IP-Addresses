...
class DataCollector():
    def __init__(self) -> None:
        ...
        if "IP_SERVERS" in os.environ:
            self.ip_servers = json.loads(os.getenv("IP_SERVERS"))
        else:
            self.ip_servers = {
    ...
    def get_request_handler(self, req:str) -> (requests.Response | None):
        try:
            ...
            raise
    def get_ip_server_data(self, url, ip):
        ip_server_data = self.get_request_handler(f'{url}/{ip}')
    ...
    def create_ip_response(self, ip):
        ...
        failure = False
        total_time = 0
        for ip_server_name, server_params in self.ip_servers.items():
            logging.info("Handling server %s with params %s", ip_server_name, server_params)
            ip_server_data = self.get_ip_server_data(server_params.get("url") ,ip)
